...
# 1. Garantir que a biblioteca Pillow está instalada para processamento de imagem
try:
...
def main():
    ...
    # Encontrar a pasta de origem válida
    src_dir = None
    ...
    if not src_dir:
        print(f"Erro: Nenhuma pasta de origem encontrada nas opções: {possible_sources}")
        sys.exit(1)
    ...
    print(f"Pasta de destino criada/confirmada: {dest_dir}\n")
    ...
    QUALITY = 80          # Qualidade do WebP (80 é excelente para web)
    MAX_WIDTH = 1920      # Carga máxima de largura para evitar lentidão no scroll
    ...
    for index, filename in enumerate(files, start=1):
        ...
        try:
            with Image.open(src_path) as img:
                ...
                # Salvar em formato WebP otimizado
                img.save(dest_path, "WEBP", quality=70, method=6, optimize=True)
